...
$url = new moodle_url('/local/coursereports/leaderboard.php');
...
$PAGE->set_title(get_string('leaderboard', 'local_coursereports'));
$PAGE->set_heading(get_string('leaderboard', 'local_coursereports'));
...
if($format){
    ...
    download_report_leaderboaed($format);
...
$dropdwonFiltter = 'Department  <select id="leaderboard-department-filter" class="form-select">';
		...
		$dropdwonFiltter .= ' Name <input type="text" id="leaderboard-name">';
		$dropdwonFiltter .= ' Email <input type="email" id="leaderboard-email"> ';
		$dropdwonFiltter .= ' <input type="button" class="btn btn-primary btn-sm" id="submit-leaderboard-filter" value="Search">';
