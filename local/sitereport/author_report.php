...
$url = $CFG->wwwroot.'/local/sitereport/author_report.php';
$PAGE->set_context(context_system::instance());
$PAGE->set_url($url);

$PAGE->requires->jquery();
//$PAGE->requires->js_call_amd('local_sitereport/sitereport', 'Init');
$PAGE->set_title(get_string('author_report', 'local_sitereport'));
$PAGE->set_pagelayout('standard');
$PAGE->set_heading( get_string('author_report', 'local_sitereport'));

$PAGE->navbar->add(get_string('allreport', 'local_sitereport'), new moodle_url($CFG->wwwroot.'/local/sitereport/'));
$PAGE->navbar->add(get_string('author_report', 'local_sitereport'), null);
...
if ($author_report_cancel) {
    $searchurl = new moodle_url($CFG->wwwroot.'/local/sitereport/author_report.php',null);
    redirect($searchurl);
...
if($format){
    ...
    download_author_report($format,$ecn); 
...
echo $renderer->author_report($page,$ecn,$startdate,$enddate);

echo $OUTPUT->download_dataformat_selector(get_string('downloadreport','local_sitereport'), $url, $name = 'dataformat', array('ecn'=>$ecn));
