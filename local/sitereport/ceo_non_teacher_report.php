...
$PAGE->set_url($CFG->wwwroot.'/local/sitereport/ceo_non_teacher_report.php', array());

$PAGE->requires->jquery();
//$PAGE->requires->js_call_amd('local_sitereport/sitereport', 'Init');
$PAGE->set_title(get_string('non_teacher_report', 'local_sitereport'));
$PAGE->set_pagelayout('standard');
$PAGE->set_heading( get_string('non_teacher_report', 'local_sitereport'));

$PAGE->navbar->add(get_string('non_teacher_report', 'local_sitereport'), null);
...
if ($ceo_author_report_cancel) {
    ...
    $searchurl = new moodle_url($CFG->wwwroot.'/local/sitereport/ceo_non_teacher_report.php',null);
    redirect($searchurl);
...
if($format){
    ceo_download_non_instructor_report($format,$dpt,$ecn,$startdate,$enddate); 
}
...
echo $renderer->ceo_non_teacher_report($page,$dpt,$ecn,$startdate,$enddate);
