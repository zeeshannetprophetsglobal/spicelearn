...
function get_el_training_type($categotryid){
    global $DB;
    $sql = 'SELECT cc.* FROM {course_categories} as cc WHERE cc.visible=1 AND
    cc.path LIKE "/'.$categotryid.'/%" /* AND cc.depth = "3" */ AND cc.idnumber LIKE "el%"';
    $trainingType = $DB->get_records_sql($sql,null);
    return $trainingType;
}

function get_elcourse($categotryid,$filterData){
...
function get_enrol_user($categoryId,$filterData){
    ...
    $totaluser = [];    
    $count = 0;
    $courseData = get_elcourse($categoryId,$filterData);
