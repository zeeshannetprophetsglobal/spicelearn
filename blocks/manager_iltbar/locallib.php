...
function get_iltcourse($categotryid){

    global $DB;
    $sql = 'SELECT c.* FROM {course} as c JOIN {course_categories} as cc ON c.category = cc.id WHERE c.visible=1 AND cc.idnumber LIKE "%ilt%" AND cc.path LIKE "%'.$categotryid.'%"';
    $coursedata = $DB->get_records_sql($sql,null);
    return $coursedata;
}

function get_enrol_user($categoryId){

    global $DB;
    $totaluser = [];    
    $count = 0;
    $courseData = get_iltcourse($categoryId);
