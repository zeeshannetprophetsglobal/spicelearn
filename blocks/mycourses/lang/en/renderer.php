...
class block_mycourses_renderer extends plugin_renderer_base
{
    ...
    public function groups_filtter($departmentid){
        ...
        $html .= html_writer::start_tag('div',array('class'=>'col-md-6 float-right'));
        ...
        $html .= html_writer::tag('option',get_string('notstart','block_mycourses'),array('value'=>2));      
        $html .= html_writer::end_tag('select');            
        $html .= html_writer::end_tag('div');
        $html .= html_writer::end_tag('div');
    ...
    public function user_courses_by_category($categoryid,$group,$status){

        global $CFG;
        $html ='';
        $courses = block_mycourses_get_user_courses_by_category($categoryid,$group);
            ...
            if($status == 1){
                ...
                if($checkstatus == 'Completed'){
                    ...
                    $table->data[] = array(
                        ...
                        html_writer::link($courselink, $course->course_name,array()),
                        $progress,
                        block_mycourses_course_status($course->id)
